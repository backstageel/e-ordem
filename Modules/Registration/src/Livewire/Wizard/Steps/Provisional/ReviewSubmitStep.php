...
class ReviewSubmitStep extends StepComponent
{
    public function summary(): array
    {
        ...
        $feeCalculation = $registrationType
            ? app(FeeCalculationService::class)->calculateForProvisional($subtype)
            : ['fees' => [], 'total' => 0.00, 'breakdown' => []];

        return [
            'subtype' => $subtype,
            'subtype_name' => $registrationType?->name,
    ...
    public function submit(): mixed
    {
        ...
        // TODO: Create ProvisionalAction to handle provisional registration creation
        // For now, redirect to success page
        // $registration = $provisionalAction->execute(...);

        session(['registration_number' => 'PROV-'.now()->year.'-0001']); // Temporary

        return redirect()->route('guest.registrations.success');
    }

    public function render()
    {
        return view('registration::livewire.registration.wizard.steps.provisional.review-submit', [
