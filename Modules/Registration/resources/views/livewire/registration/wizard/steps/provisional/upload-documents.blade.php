<div class="card card-spacing" x-data="{progress: 0}">
    ...
    <div class="card-body card-spacing">
        @php($seedFirst = (array) $this->state()->forStepClass(\Modules\Registration\Livewire\Wizard\Steps\Provisional\ChooseSubtypeStep::class))
        ...
        @if(count($this->commonDocuments) > 0)
            <h5 class="heading-3 mb-spacing-3">Documentos Comuns (13 obrigatórios)</h5>
            <div class="list-group mb-spacing-4">
                @foreach ($this->commonDocuments as $doc)
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <input
                            ...
                            wire:change="queueDocument('{{ $doc }}', $event.target.files[0])"
        ...
        @if(count($this->specificDocuments) > 0)
            <h5 class="heading-3 mb-spacing-3">Documentos Específicos do Subtipo</h5>
            <div class="list-group mb-spacing-3">
                @foreach ($this->specificDocuments as $doc)
                    <div class="list-group-item d-flex flex-column flex-md-row align-items-center justify-content-between gap-3">
                        <div>
                            <strong class="text-base">{{ $this->getDocumentLabel($doc) }}</strong>
                            @if (isset($uploads[$doc]))
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <input
                                ...
                                style="width: 280px"
                            />
                            <button type="button" class="btn btn-outline-secondary btn-sm" onclick="document.getElementById('file-{{ $doc }}').click()" aria-label="Selecionar {{ $this->getDocumentLabel($doc) }}">
                                <i class="fas fa-folder-open me-1" aria-hidden="true"></i>Selecionar
                            </button>
                            <button type="button" class="btn btn-primary btn-sm" wire:click="uploadCurrent" @disabled($currentDocumentKey !== '{{ $doc }}' || !$currentFile) aria-label="Carregar {{ $this->getDocumentLabel($doc) }}">
                                <i class="fas fa-upload me-1" aria-hidden="true"></i>Carregar
                            </button>
                            @if(isset($uploads[$doc]))
                                <span class="text-success small">
                ...
                <div class="card-body card-spacing d-flex flex-column flex-md-row align-items-center justify-content-between gap-3">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <input
                            ...
                            wire:model="currentFile"
                            wire:change="queueDocument('payment_proof', $event.target.files[0])"
                            accept=".pdf,.jpg,.jpeg,.png"
                            x-on:livewire-upload-progress="progress = $event.detail.progress"
                            x-on:livewire-upload-finish="progress = 0"
                            x-on:livewire-upload-error="progress = 0"
