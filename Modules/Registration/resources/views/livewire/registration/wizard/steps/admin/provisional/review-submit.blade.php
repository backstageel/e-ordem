        ...
        <div class="row g-3">
            <div class="col-md-6">
                <div class="border rounded p-spacing-3 h-100">
                    ...
                    <div class="text-base mb-2"><strong>Subtipo:</strong> {{ $summary['subtype_name'] ?? '-' }}</div>
                    <div class="text-base"><strong>Tipo:</strong> {{ $summary['type'] ?? '-' }}</div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="border rounded p-spacing-3 h-100">
                    <h6 class="heading-6 text-muted mb-3">Contactos</h6>
                    <dl class="row mb-0">
                        @foreach(($summary['contact'] ?? []) as $label => $value)
                            <dt class="col-5 text-sm text-muted">{{ $label }}</dt>
                            <dd class="col-7 text-base">{{ $value }}</dd>
                        @endforeach
                    </dl>
                </div>
            </div>
            <div class="col-md-6">
                <div class="border rounded p-spacing-3 h-100">
                    <h6 class="heading-6 text-muted mb-3">Dados Pessoais</h6>
                    <dl class="row mb-0">
                        @foreach(($summary['personal'] ?? []) as $label => $value)
                            <dt class="col-5 text-sm text-muted">{{ $label }}</dt>
                            <dd class="col-7 text-base">{{ $value }}</dd>
                        @endforeach
                    </dl>
            ...
            <div class="col-md-6">
                <div class="border rounded p-spacing-3 h-100">
                    <h6 class="heading-6 text-muted mb-3">Identificação e Morada</h6>
                    <dl class="row mb-0">
                        @foreach(($summary['identity'] ?? []) as $label => $value)
                            <dt class="col-5 text-sm text-muted">{{ $label }}</dt>
                            <dd class="col-7 text-base">{{ $value }}</dd>
                        @endforeach
                    </dl>
            ...
            <div class="col-md-6">
                <div class="border rounded p-spacing-3 h-100">
                    <h6 class="heading-6 text-muted mb-3">Dados Académicos e Profissionais</h6>
                    <dl class="row mb-0">
                        @foreach(($summary['academic'] ?? []) as $label => $value)
                            <dt class="col-5 text-sm text-muted">{{ $label }}</dt>
                            <dd class="col-7 text-base">{{ $value }}</dd>
                        @endforeach
                    </dl>
