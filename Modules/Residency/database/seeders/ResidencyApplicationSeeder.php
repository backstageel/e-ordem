...
namespace Modules\Residency\Database\Seeders;
...
class ResidencyApplicationSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // If no programs exist, skip seeding applications
        if ($programs->isEmpty()) {
            $this->command->info('No residency programs found. Skipping residency application seeding.');
        ...
        foreach ($members as $index => $member) {
            // Assign each member to 1-2 programs randomly, but not more than available
            $maxPrograms = min(2, $programs->count());
            $programCount = rand(1, $maxPrograms);
